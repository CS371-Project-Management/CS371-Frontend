"use client";

import { useState } from "react";
import ReportFail from "@/components/modals/report/ReportFail";
import { CourseTypesCreate } from "@/types/courseTypes";
import { CourseService } from "@/services/courseService";

interface ModalProps {
    isOpen: boolean;
    onClose: () => void;
    class_id : string;
}

const difficultyLevels = ["easy", "medium", "hard"];

export default function ModalCreateCourse({ isOpen, onClose, class_id }: ModalProps) {
    if (!isOpen) return null;

    const [courseName, setCourseName] = useState("");
    const [description, setDescription] = useState("");
    const [number, setNumber] = useState(1);
    const [difficulty, setDifficulty] = useState("Easy");
    const [errors, setErrors] = useState<{ courseName?: string; description?: string; number?: string }>({});
    const [isFail, setIsFail] = useState(false);

    const handleSave = async () => {
        let newErrors: { courseName?: string; description?: string; number?: string } = {};

        if (!courseName.trim()) {
            newErrors.courseName = "Course name is required.";
        }
        if (!description.trim()) {
            newErrors.description = "Description is required.";
        }
        if (number < 1) {
            newErrors.number = "Number must be at least 1.";
        }

        if (Object.keys(newErrors).length > 0) {
            setErrors(newErrors);
            return;
        }
        setErrors({});

        const req: CourseTypesCreate = {
            title: courseName,
            description,
            class_id: class_id,
            difficulty_level: difficulty,
            number,
        };
        try {
            await CourseService.createCourse(req);
            onClose();
        } catch (error) {
            console.error("Error creating class:", error);
            setIsFail(true);
        }
    };

    return (
        <div className="fixed inset-0 z-1 flex items-center justify-center bg-black/50 backdrop-blur-sm text-black">
            <div className="bg-white p-6 rounded-2xl shadow-xl w-[600px]">
                <h2 className="text-xl font-bold mb-4">CREATE NEW COURSE</h2>

                <div className="mb-4">
                    <label className="block text-sm font-bold mb-1">Course name <span className="text-red-500">*</span></label>
                    <input
                        type="text"
                        value={courseName}
                        onChange={(e) => setCourseName(e.target.value)}
                        placeholder="Course name"
                        className={`text-sm w-full p-3 border rounded-lg focus:outline-none focus:ring-2 ${errors.courseName ? "border-red-500 focus:ring-red-500" : "border-gray-300 focus:ring-blue-500"}`}
                    />
                    {errors.courseName && <p className="text-red-500 text-xs mt-1">{errors.courseName}</p>}
                </div>

                <div className="mb-4">
                    <label className="block text-sm font-bold mb-1">Description <span className="text-red-500">*</span></label>
                    <textarea
                        value={description}
                        onChange={(e) => setDescription(e.target.value)}
                        placeholder="Description"
                        className={`text-sm w-full p-3 border rounded-lg focus:outline-none focus:ring-2 resize-none h-24 ${errors.description ? "border-red-500 focus:ring-red-500" : "border-gray-300 focus:ring-blue-500"}`}
                    ></textarea>
                    {errors.description && <p className="text-red-500 text-xs mt-1">{errors.description}</p>}
                </div>

                <div className="flex justify-between mb-4">
                    <div>
                        <label className="block text-sm font-bold mb-1">Difficulty Level</label>
                        <select
                            value={difficulty}
                            onChange={(e) => setDifficulty(e.target.value)}
                            className="text-sm w-full p-3 border rounded-lg focus:outline-none focus:ring-2 border-gray-300 focus:ring-blue-500"
                        >
                            {difficultyLevels.map((level) => (
                                <option key={level} value={level}>{level.toLocaleUpperCase()}</option>
                            ))}
                        </select>
                    </div>
                    <div>
                        <label className="block text-sm font-bold mb-1">Number of this course</label>
                        <input
                            type="number"
                            value={number}
                            onChange={(e) => setNumber(Math.max(1, Number(e.target.value)))}
                            className={`text-sm w-full p-2 border rounded-lg focus:outline-none focus:ring-2 ${errors.number ? "border-red-500 focus:ring-red-500" : "border-gray-300 focus:ring-blue-500"}`}
                        />
                        {errors.number && <p className="text-red-500 text-xs mt-1">{errors.number}</p>}
                    </div>
                </div>

                <div className="flex justify-between">
                    <button onClick={onClose} className="px-4 py-2 bg-gray-200 text-black rounded-lg hover:bg-gray-300">Cancel</button>
                    <button onClick={handleSave} className="px-4 py-2 bg-black text-white rounded-lg hover:bg-gray-800">Save</button>
                </div>
            </div>

            <ReportFail
                isOpen={isFail}
                onClose={() => setIsFail(false)}
                title="Failed to create course"
                press="OK"
            />
        </div>
    );
};
